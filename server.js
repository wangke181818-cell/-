...
// 让 /uploads/... 可访问（头像 URL 就是 /uploads/avatars/xxx.png）
app.use("/uploads", express.static(UPLOADS_DIR));

// ===== multer（先 storage 再 upload，避免你之前的崩溃）=====
...
try {
  ...
  if (!hasPassword) db.prepare("ALTER TABLE users ADD COLUMN password TEXT").run();
} catch (e) {
...
try {
  ...
  if (!hasAvatar) db.prepare("ALTER TABLE users ADD COLUMN avatar_url TEXT").run();
} catch (e) {
...
// disabled_default_cards 表（你之前是“只对自己生效”，现在我们让“绑定的任意一方隐藏，全体生效”）
db.prepare(`
...
// 获取 user 的“绑定圈子”（自己+所有绑定对象）
function getPartnerIdsIncludingSelf(userId) {
  ...
  if (!partners.includes(userId)) partners.push(userId);
  return partners;
}

// 抽卡用池：默认卡过滤 = “绑定圈子里任何人隐藏的默认卡都不出现”
function getUserCardPoolForDraw(userId) {
  const pool = {};
  const rarities = Object.keys(defaultCardPool);
  const allIds = getPartnerIdsIncludingSelf(userId);
...
app.post("/api/profile/avatar/upload", upload.single("avatar"), (req, res) => {
  try {
    ...
    if (!userId || !req.file) return res.status(400).json({ error: "缺少 userId 或 文件" });
...
app.post("/api/profile/avatar", (req, res) => {
  ...
  if (!userId || typeof avatarUrl !== "string") return res.status(400).json({ error: "缺少参数" });

  const user = db.prepare("SELECT * FROM users WHERE id = ?").get(userId);
  if (!user) return res.status(404).json({ error: "用户不存在" });

  db.prepare("UPDATE users SET avatar_url = ? WHERE id = ?").run(avatarUrl.trim(), userId);
  res.json({ message: "头像已更新", avatar_url: avatarUrl.trim() });
});
...
app.post("/api/bind", (req, res) => {
  ...
  if (partner.id === user.id) return res.status(400).json({ error: "不能绑定自己" });
  ...
  if (!exists) db.prepare("INSERT INTO couples (user_a_id, user_b_id) VALUES (?, ?)").run(user.id, partner.id);

  res.json({
    message: exists ? "已经绑定过了" : "绑定成功！",
    partner: { id: partner.id, name: partner.name, draw_count: partner.draw_count, avatar_url: partner.avatar_url || "" },
  });
});
...
app.get("/api/partners", (req, res) => {
  ...
  const couples = db.prepare(`SELECT * FROM couples WHERE user_a_id = ? OR user_b_id = ?`).all(userId, userId);
...
app.post("/api/request-draw", (req, res) => {
  ...
  if (!isCoupled(userId, partnerId)) return res.status(400).json({ error: "你们还没有绑定，不能发起抽卡申请" });
...
app.post("/api/approve-draw", (req, res) => {
  ...
  if (request.partner_id !== Number(userId)) return res.status(403).json({ error: "你不是这个请求的另一方" });
...
app.post("/api/draw", (req, res) => {
  ...
  if (!request) return res.status(400).json({ error: "没有可用的已同意抽卡请求" });

  const card = drawCardForUser(Number(userId));
  ...
  db.prepare(`INSERT INTO draw_logs (user_id, card_text, rarity, created_at) VALUES (?, ?, ?, datetime('now'))`)
    .run(userId, card.text, card.rarity);

  db.prepare(`INSERT INTO user_cards (user_id, card_text, rarity, used, created_at) VALUES (?, ?, ?, 0, datetime('now'))`)
    .run(userId, card.text, card.rarity);

  const updatedUser = db.prepare("SELECT id, name, draw_count, avatar_url FROM users WHERE id = ?").get(userId);

  res.json({ message: "抽卡成功", card, user: updatedUser });
});
...
app.get("/api/partner-cards", (req, res) => {
  ...
  if (!userId || !partnerId) return res.status(400).json({ error: "缺少参数" });
  if (!isCoupled(userId, partnerId)) return res.status(403).json({ error: "未绑定，不能查看" });
...
app.post("/api/custom-cards/add", (req, res) => {
  ...
  if (!allowed.includes(rarity)) return res.status(400).json({ error: "rarity 必须是 SSR/SR/R/N" });

  db.prepare(`INSERT INTO custom_cards (user_id, rarity, text, enabled) VALUES (?, ?, ?, 1)`)
...
app.post("/api/custom-cards/delete", (req, res) => {
  ...
  if (!card) return res.status(404).json({ error: "找不到这张自定义卡" });

  db.prepare("UPDATE custom_cards SET enabled = 0 WHERE id = ?").run(cardId);
  res.json({ message: "已删除自定义卡" });
});

// 卡库一览（默认卡 disabled 现在是“绑定圈子共享”）
app.get("/api/card-pool", (req, res) => {
  const userId = Number(req.query.userId);
  if (!userId) return res.status(400).json({ error: "缺少 userId" });

  const partnerIds = getPartnerIdsIncludingSelf(userId); // 自己+绑定对象
  ...
  const pool = {};
  for (const rarity of Object.keys(defaultCardPool)) {
    pool[rarity] = defaultCardPool[rarity].map(text => ({
      type: "default",
      rarity,
      text,
...
// 隐藏默认卡（仍然记录是谁隐藏的，但效果对绑定圈子共享）
app.post("/api/default-cards/disable", (req, res) => {
  const { userId, rarity, text } = req.body;
  if (!userId || !rarity || !text) return res.status(400).json({ error: "缺少参数" });

  if (!(defaultCardPool[rarity] || []).includes(text)) {
    return res.status(400).json({ error: "只能隐藏系统默认卡" });
  }

  db.prepare(`INSERT OR IGNORE INTO disabled_default_cards (user_id, rarity, text) VALUES (?, ?, ?)`)
    .run(userId, rarity, text);

  res.json({ message: "已隐藏该默认卡（绑定圈子共享生效）" });
});

// 恢复默认卡（只恢复“自己隐藏”的那条记录）
app.post("/api/default-cards/enable", (req, res) => {
  const { userId, rarity, text } = req.body;
  if (!userId || !rarity || !text) return res.status(400).json({ error: "缺少参数" });

  db.prepare(`DELETE FROM disabled_default_cards WHERE user_id = ? AND rarity = ? AND text = ?`)
    .run(userId, rarity, text);

  res.json({ message: "已恢复该默认卡（如果其他绑定对象也隐藏了，它仍会隐藏）" });
});
